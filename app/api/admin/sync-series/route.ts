...
export async function POST(req: NextRequest) {
  ...
  for (const ub of userBooks) {
    const cb = ub.catalog_books as unknown as { title: string; author: string; cover_url: string } | null;
    const title  = ub.title_override  ?? cb?.title  ?? "";
    const author = ub.author_override ?? cb?.author ?? "";
    const book = { id: ub.id, title, author, status: ub.status, coverUrl: cb?.cover_url ?? "" };
