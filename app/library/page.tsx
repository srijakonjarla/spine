...
function InlineAdd({
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="text-xs text-[var(--fg-faint)] hover:text-[var(--fg-muted)] transition-colors mt-2">
        + add
      </button>
    );
  }
  ...
  return (
    <div className="mt-2">
      <CatalogSearch value={value} onChange={setValue} onSelect={handleAdd} onSubmit={() => handleAdd()} placeholder={placeholder} disabled={adding} libraryEntries={libraryEntries} />
      {value.trim() && !adding && <p className="hint-text">↵ to add · esc to cancel</p>}
    </div>
  );
}

function ShelfDivider({ year, count }: { year: number; count: number }) {
  return (
    <div className="flex items-center gap-4 my-8">
      ...
      <p className="font-[family-name:var(--font-playfair)] text-[17px] italic text-[var(--fg-heading)] shrink-0">
        {year}
        <span className="font-[family-name:var(--font-caveat)] text-[14px] font-normal not-italic ml-3 text-terra">
...
export default function LibraryPage() {
  ...
  const yearGroups = Array.from(yearMap.entries())
    .sort((a, b) => b[0] - a[0])
    .map(([year, books]) => ({ year, books: books.sort((a, b) => (b.dateFinished ?? "").localeCompare(a.dateFinished ?? "")) }));
      ...
      <div className="page-content">
        ...
        <div className="flex items-baseline justify-between mb-6">
          ...
          {!loading && (
            <div className="flex items-center gap-3">
              <button onClick={() => setView("grid")} className={`text-xs px-2 py-1 rounded transition-colors ${view === "grid" ? "bg-[var(--bg-hover)] text-[var(--fg)]" : "text-[var(--fg-faint)]"}`}>▦</button>
              <button onClick={() => setView("list")} className={`text-xs px-2 py-1 rounded transition-colors ${view === "list" ? "bg-[var(--bg-hover)] text-[var(--fg)]" : "text-[var(--fg-faint)]"}`}>☰</button>
              <span className="text-xs text-[var(--fg-faint)]">{entries.length} books</span>
            </div>
          )}
        </div>
        ...
        {allMoods.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            {allMoods.map((mood) => (
              <MoodChip key={mood} mood={mood} active={activeMood === mood} onClick={() => setActiveMood(activeMood === mood ? null : mood)} />
            ))}
          </div>
        )}
            ...
            <div className="mb-6">
              ...
              <div className="space-y-0.5">
                {currentlyReading.map((e) => (
                  <Link key={e.id} href={`/book/${e.id}`} className="flex items-center gap-3 py-1.5 px-2 -mx-2 rounded-lg hover:bg-[var(--bg-plum-trace)] transition-colors group">
                    <span className="text-xs shrink-0 text-terra">○</span>
                    <span className="text-sm flex-1 truncate text-[var(--fg)]">{e.title}</span>
                    {e.author && <span className="text-xs shrink-0 hidden sm:block text-[var(--fg-faint)]">{e.author}</span>}
                  </Link>
                ))}
              </div>
              <InlineAdd placeholder="what are you reading?" onAdd={(c, r) => addBook("reading", c, r)} libraryEntries={entries} />
            </div>
          )}
          ...
          {wantToRead.length > 0 && (
            <div>
              <div className="flex items-baseline justify-between mb-3">
                ...
                <Link href="/library/want-to-read" className="text-xs text-[var(--fg-faint)] hover:text-[var(--fg-muted)] transition-colors">
                  all {entries.filter(e => e.status === "want-to-read").length} →
                </Link>
              </div>
              <div className="space-y-0.5">
                {wantToRead.slice(0, 8).map((e) => (
                  <Link key={e.id} href={`/book/${e.id}`} className="flex items-center gap-3 py-1.5 px-2 -mx-2 rounded-lg hover:bg-[var(--bg-plum-trace)] transition-colors group">
                    <span className="text-xs shrink-0 text-[var(--fg-faint)]">◌</span>
                    <span className="text-sm flex-1 truncate text-[var(--fg)]">{e.title}</span>
                    {e.author && <span className="text-xs shrink-0 hidden sm:block text-[var(--fg-faint)]">{e.author}</span>}
                  </Link>
                ))}
              </div>
              <InlineAdd placeholder="add to tbr..." onAdd={(c, r) => addBook("want-to-read", c, r)} libraryEntries={entries} />
            </div>
          )}
        ...
        {!loading && yearGroups.length === 0 && !search && !activeMood && !activeGenre && (
          <p className="text-xs text-[var(--fg-faint)]">no finished books yet.</p>
        )}

        {!loading && yearGroups.map(({ year, books }) => (
          <div key={year}>
            ...
            {view === "grid" ? (
              ...
              <div className="space-y-0.5">
                {books.map((e) => (
                  <Link key={e.id} href={`/book/${e.id}`} className="flex items-center gap-3 py-1.5 px-2 -mx-2 rounded-lg hover:bg-[var(--bg-plum-trace)] transition-colors group">
                    <BookCoverThumb coverUrl={e.coverUrl} title={e.title} width="w-6" height="h-9" />
                    <span className="text-sm flex-1 truncate text-[var(--fg)]">{e.title || "untitled"}</span>
                    {e.author && <span className="text-xs shrink-0 hidden sm:block text-[var(--fg-faint)]">{e.author}</span>}
