...
export function parseGoodreadsCSV(text: string): GoodreadsPreview[] {
  ...
  return rows
    .map((row) => {
      ...
      const rawIsbn13 = (row["ISBN13"] ?? row["ISBN"] ?? "")
        .replace(/[="]/g, "")
        .trim();
      ...
      const entry: BookEntry = {
        ...
        dateShelved: status === "want-to-read" ? dateAdded : "",
        dateDnfed: isDNF ? dateRead : "",
        rating,
