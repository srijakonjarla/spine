...
function matchLibraryBook(
  ...
  return library.find(
    (b) => b.title.toLowerCase() === seriesBook.title.toLowerCase(),
  );
}
...
export default function SeriesCard({
  ...
  return (
    <div className="border border-line rounded-xl p-6">
      ...
      {total > 0 && (
        <ProgressBar value={readCount / total} size="xs" className="mb-4" />
      )}
          ...
          return (
            ...
            >
              ...
              {!libraryBook && book.status !== "unread" && (
                <span
                  className={`text-xs shrink-0 ${STATUS_TEXT_CLS[book.status]}`}
                >
                  {book.status}
                </span>
              )}
